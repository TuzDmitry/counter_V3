...
type ActionType = any | SetInitialDataSuccessType | UpdateMaxValueSuccessType | UpdateStartValueSuccessType|UpdateMemoryValueSuccessType|ToggleModeType
...
export const toggleModeThunk=  (val: boolean) => {
    return (dispatch: Dispatch<ActionType>, getState:()=>AppRootState) => {
       if(!val){
           APIservice.updateCounterMemoryValue(getState().counterReducer.startValue)
               .then(data => {
                   debugger
...
export const incrementMemoryValue=()=>{
    return (dispatch:Dispatch<ActionType>, getState:()=>AppRootState)=>{
        ...
        if(newVal<=maxVal){
            APIservice.updateCounterMemoryValue(newVal)
                .then(data=>{
                    dispatch(updateMemoryValueSuccess(data.memoryValue))
                })
        }
